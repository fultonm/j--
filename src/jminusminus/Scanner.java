	...
	public TokenInfo getNextToken() {
				...
				if (ch == '/') {
					...
					while (ch != EOFCH) {
						...
						if (ch == '*') {
							nextCh();
							if (ch == '/') {
								/** End of the multiline comment detected */
								nextCh();
								break;
							}
						}
		...
		case '>':
			if (ch == '=') {
				...
				if (ch == '=') {
					nextCh();
					return new TokenInfo(BWRSHIFT_ASSIGN, line);
				} else if (ch == '>') {
					nextCh();
					return new TokenInfo(BWZEROFILLRSHIFT, line);
				} else {
					nextCh();
					return new TokenInfo(BWRSHIFT, line);
				}
			} else {
				nextCh();
		...
		case '<':
			...
			if (ch == '=') {
				...
				if (ch == '=') {
					nextCh();
					return new TokenInfo(BWLSHIFT_ASSIGN, line);
				} else {
					nextCh();
					return new TokenInfo(BWLSHIFT, line);
				}
			} else {
				nextCh();
		...
		case '?':
			nextCh();
			return new TokenInfo(TERNARY_TEST, line);
		case ':':
			nextCh();
			return new TokenInfo(TERNARY_ELSE, line);
		case '\'':
