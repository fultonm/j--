...
public class Parser {
    ...
    private boolean seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            if (have(BOOLEAN) || have(CHAR) || have(INT) || have(DOUBLE) || have(FLOAT)
                    || have(LONG)) {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            return new JDoWhileStatement(line, condition, statement);
        } else if (have(RETURN)) {
            if (have(SEMI)) {
            ...
            JBlock body = block();
            return new JSwitchStatement(line, condition, body);
        } else if (have(TRY)) {
            JBlock tryBlock = block();
            //TODO: There can be multiple CATCH blocks for each type of Exception, FileNotFoundException, BadArgumentException, e.t.c for finding a file
            mustBe(CATCH);
            JBlock catchBlock = block();
            if (have(FINALLY)) {
                JBlock finallyBlock = block();
                return new JTryStatement(line, tryBlock, catchBlock, finallyBlock);
            }
     ...
     *       conditionalAndExpression // level 1
     ...
     *
     * <pre>
     *   ternaryExpression ::= conditionalOrExpression // level 2
     *
     * </pre>
     *
     ...
     *
     * <pre>
     *   conditionalOrExpression ::= conditionalAndExpression // level 3
     ...
     *
     * <pre>
     *   conditionalAndExpression ::= bitwiseOrExpression // level 4
    ...
    /**
     * Parse an bitwise OR expression.
     *
     * <pre>
    ...
    private JExpression bitwiseOrExpression() {
        ...
        while (more) {
            if (have(BWOR)) {
                lhs = new JBWOrOp(line, lhs, bitwiseXorExpression());
            } else {
                more = false;
            }
        }
    ...
    /**
     * Parse an bitwise XOR expression.
     *
     * <pre>
     ...
     * </pre>
     *
     * @return an AST for an bitwiseXorExpression.
     */
    ...
    private JExpression bitwiseXorExpression() {
        ...
        while (more) {
            if (have(BWXOR)) {
                lhs = new JBWXorOp(line, lhs, bitwiseAndExpression());
            } else {
                more = false;
            }
        }
     ...
     * </pre>
     *
     * @return an AST for an bitwiseAndExpression.
     */
     ...
     *
     * <pre>
     *   equalityExpression ::= relationalExpression  // level 8
     ...
     *
     * <pre>
     *   relationalExpression ::= shiftingExpression  // level 9
    ...
    private JExpression relationalExpression() {
        int line = scanner.token().line();
        JExpression lhs = shiftingExpression();
        if (have(GT)) {
            return new JGreaterThanOp(line, lhs, shiftingExpression());
        } else if (have(LT)) {
            return new JLessThanOp(line, lhs, shiftingExpression());
        } else if (have(GE)) {
            return new JGreaterEqualOp(line, lhs, shiftingExpression());
        } else if (have(LE)) {
            return new JLessEqualOp(line, lhs, shiftingExpression());
    ...
    /**
     * Parse a shifting expression.
     *
     * <pre>
     *   shiftingExpression ::= additiveExpression  // level 10
     *                              [(<< | >> | >>>) additiveExpression
     *                              | INSTANCEOF referenceType]
     * </pre>
     *
     * @return an AST for a shiftingExpression.
     */

    private JExpression shiftingExpression() {
        int line = scanner.token().line();
        JExpression lhs = additiveExpression();
        if (have(BWLSHIFT)) {
            return new JBWLShiftOp(line, lhs, additiveExpression());
        } else if (have(BWRSHIFT)) {
            return new JBWRShiftOp(line, lhs, additiveExpression());
        } else if (have(BWZEROFILLRSHIFT)) {
            return new JBWRShiftZeroFillOp(line, lhs, additiveExpression());
        } else {
            return lhs;
        }
    }
     ...
     *
     * <pre>
     *   additiveExpression ::= multiplicativeExpression // level 11
     ...
     *
     * <pre>
     *   multiplicativeExpression ::= unaryExpression  // level 12
     ...
     *
     * <pre>
     *   unaryExpression ::= INC unaryExpression // level 13
     ...
     *
     * <pre>
     *   simpleUnaryExpression ::= LNOT unaryExpression // A part of unaryExpression, Still level 13
     ...
     *
     * <pre>
     *   postfixExpression ::= primary {selector} {DEC} // 14
     * </pre>
     *
     ...
     *
     * <pre>
     *   selector ::= DOT qualifiedIdentifier [arguments] // Level 15
    ...
    private JExpression primary() {
        ...
        if (see(LPAREN)) {
            ...
            if (see(LPAREN)) {
                return new JMessageExpression(line, null, ambiguousPart(id), id.simpleName(),
                        arguments());
